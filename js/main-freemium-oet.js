// SpellRightPro Freemium OET Trainer - Complete, fixed version
...
let score = 0;
let userAnswers = [];
let sessionType = "test"; // "test" or "practice"
let flaggedWords = [];
...
function loadOETWords() {
  if (window.oetWords) {
    oetWords = window.oetWords;
    return;
  }
  // Fallback: load from file (classic global)
  fetch('js/oet_word_list.js')
    .then(resp => resp.text())
    .then(js => {
      const match = js.match(/\[\s*["'][^]+?\]/);
      oetWords = match ? eval(match[0]) : [];
      window.oetWords = oetWords;
    });
}
loadOETWords();
...
addCustomBtn.onclick = () => {
  if (usedCustomListToday) {
    alert("You can only use one custom list per day in freemium!");
    return;
  }
  let input = customInput.value.trim();
  let customWords = input.split(/[\s,;]+/).map(w => w.trim()).filter(w => w);
  if (customWords.length === 0) {
    alert("Please paste or type custom words!");
    return;
  }
  words = customWords;
...
function showWord() {
  ...
  document.getElementById('check-btn').onclick = () => {
    ...
    if (userInput.toLowerCase() === word.toLowerCase()) score++;
    currentIndex++;
    showWord();
  };
