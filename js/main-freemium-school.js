// main-freemium-school.js - Freemium School Mode (derived from OET, capped at 10/day)
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Default School List loaded from school.json
  let schoolWords = [];
  (async () => {
    try {
      const res = await fetch('school.json', { cache: 'no-cache' });
      const data = await res.json();
      if (Array.isArray(data?.words)) schoolWords = data.words.filter(Boolean);
  ...
  function startSession() {
    if (words.length === 0 && !usedCustomListToday) {
      words = Array.isArray(schoolWords) ? [...schoolWords] : [];
    }
    if (sessionMode === "test") {
      words = shuffleArray([...words]).slice(0, 24);
    }
    // Apply daily cap
    words = capWordsForToday("School", words);
  ...
  function endSession() {
    ...
    setUsedToday("School", usedToday("School") + words.length);
  ...
  function resetWordList() {
    ...
    fileInput.value = '';
    summaryArea.classList.add('hidden');
    showAlert("Word list cleared. Add new words or use default School list.", 'info');
    // default list: school.json loaded at boot, used at start
  }
