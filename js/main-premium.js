...
(function () {
  ...
  const overlay = $('#authOverlay'),
        btnGoogle = $('#btnGoogle'),
        btnLogin = $('#btnEmailLogin'),
        btnSignup = $('#btnEmailSignup'),
        emailEl = $('#authEmail'),
        passEl = $('#authPass'),
        msgEl = $('#authMsg'),
  ...
  function msg(t, good = false) {
    if (!msgEl) return;
    msgEl.style.color = good ? '#7b2ff7' : '#aaa';
    msgEl.textContent = t || '';
  }
  ...
  // Switch Login/Register tabs inside overlay
  function setAuthMode(m) {
    const login = m === 'login';
    tabLogin?.classList.toggle('active', login);
    tabSignup?.classList.toggle('active', !login);
    btnLogin?.classList.toggle('hidden', !login);
    btnSignup?.classList.toggle('hidden', login);
    toggleText && (toggleText.textContent = login
  ...
  // Email signup (verification optional – does not block)
  ...
  // Single auth state observer (fixes previous duplication & freeze)
  ...
  (function Bee() {
    ...
    async function initRec() {
      await pause(300); // give AudioGuards time to prime
    ...
    async function loadList() {
      ...
      try {
        ...
        master = ['accommodate', 'rhythm', 'occurrence', 'necessary', 'embarrass'];
      }
    }
  ...
  (function School() {
    ...
    async function loadList() {
      ...
      try {
        ...
        master = ['apple', 'banana', 'computer', 'library', 'mountain'];
      }
    }
    function reset() { idx = 0; correct = 0; total = 0; incorrect = []; flags.clear(); els.summary.innerHTML = ''; }
    ...
    function grade() {
      ...
      total++;
      if (ok) { correct++; setText(els.feedback, '✅'); }
      else { incorrect.push(t); setText(els.feedback, `❌ ${t}`); }
    ...
    els.input?.addEventListener('keydown', e => {
      if (e.key === 'Enter') { e.preventDefault(); if (requireAuth()) grade(); }
    });
    els.flag?.addEventListener('click', () => {
      if (!requireAuth()) return;
      const w = list[idx]; if (!w) return;
  ...
  (function OET() {
    ...
    async function loadList() {
      if (master.length) return;
      // Prefer global OET_WORDS if defined (no ES module "export" allowed)
      if (Array.isArray(window.OET_WORDS) && window.OET_WORDS.length) {
        master = window.OET_WORDS.slice();
        return;
      }
      // Retry a few times in case oet_word_list.js loads a bit late
      for (let i = 0; i < 10 && (!window.OET_WORDS || !window.OET_WORDS.length); i++) {
        await pause(300);
      }
      if (Array.isArray(window.OET_WORDS) && window.OET_WORDS.length) {
        master = window.OET_WORDS.slice();
      } else {
        master = ['abdomen', 'anemia', 'antibiotic', 'artery', 'asthma', 'biopsy', 'catheter', 'diagnosis', 'embolism', 'fracture'];
      }
    }
    function reset() { idx = 0; correct = 0; total = 0; incorrect = []; flags.clear(); els.summary.innerHTML = ''; }
    function progress() { setText(els.progress, `Word ${idx + 1} of ${list.length}`); els.input?.focus(); }
