// Premium page: Firebase auth + promo + checkout redirect (update endpoint!)
(function(){
  ...
  // Promo window (adjust dates/prices as needed)
  ...
  function updatePromo(){
    ...
    if (diff>0){
      ...
      const hrs=Math.floor(diff/1000/60/60), mins=Math.floor((diff/1000/60)%60);
  ...
  updatePromo(); setInterval(updatePromo, 60000);
  ...
  if (firebase?.auth) {
    firebase.auth().onAuthStateChanged(u=>{ currentUser=u; updateUI(); });
  }
  ...
  upgradeBtn.addEventListener('click', async ()=>{
    ...
    upgradeBtn.innerHTML='<i class="fa fa-spinner fa-spin"></i> Redirecting…'; upgradeBtn.disabled=true;
    try{
      const res = await fetch(CHECKOUT_ENDPOINT, {
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ uid: currentUser.uid, plan: 'annual', promoActive: promoEnd - new Date() > 0 })
      });
      if (!res.ok) throw new Error('Checkout API error');
      const data = await res.json();
      if (data?.url) { window.location.href = data.url; }
      else { throw new Error('No URL from checkout'); }
    }catch(err){
      alert('Could not start checkout. Please try again or contact support.');
      upgradeBtn.innerHTML='<i class="fa fa-arrow-up-right-from-square"></i> Upgrade Now'; upgradeBtn.disabled=false;
    }
  });
})();
