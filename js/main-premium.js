// main-premium.js — Premium app with ad gating (Auto Ads only for non-premium)
...
function initStripe() {
  ...
  if (!/^pk_(test|live)_/.test(key)) {
    console.warn('Stripe publishable key missing/invalid. Set stripeConfig.publicKey in config.js');
    return;
  }
  stripe = Stripe(key);
}
...
function loadAutoAds() {
  ...
  s.setAttribute('data-ad-client', 'ca-pub-7632930282249669');
...
function checkUrlForPaymentStatus() {
  ...
  if (urlParams.has('payment_success')) {
    ...
    // small delay to re-fetch premium status cleanly
    setTimeout(() => location.reload(), 400);
  }
}

// ==================== FIRESTORE & PREMIUM ====================
async function ensureUserDoc(user, trialDays = (window.appConfig?.trialDays || 7)) {
  const ref = firebase.firestore().collection('users').doc(user.uid);
  let snap;
  try { snap = await ref.get(); } catch (err) { console.error(err); showAlert('Could not access your profile.', 'error', 5000); throw err; }
  if (!snap.exists) {
    const future = firebase.firestore.Timestamp.fromDate(new Date(Date.now() + trialDays*24*3600*1000));
    await ref.set({
      email: user.email || '',
      isPremium: false,
      trialEndsAt: future,
  ...
  if (!data.trialEndsAt || typeof data.trialEndsAt.toDate !== 'function') {
    updates.trialEndsAt = firebase.firestore.Timestamp.fromDate(new Date(Date.now() + trialDays*24*3600*1000));
  }
...
async function checkPremiumStatus() {
  ...
  try {
    ...
    const data = snap.data();
    const hasTrial = data.trialEndsAt?.toDate && (data.trialEndsAt.toDate() > new Date());
    premiumUser = data.isPremium === true || !!hasTrial;

    if (premiumUser) disableAutoAdsNow();
    else loadAutoAds();
...
function initAuthState() {
  firebase.auth().onAuthStateChanged(async (user) => {
    ...
    if (!user) {
      renderAuthLoggedOut();
      loadAutoAds(); // show ads while logged out
      return;
    }
    ...
    renderAuthLoggedIn();
    if (premiumUser) renderExamUI();
    else showPremiumUpsell();
  });
}
...
async function handleSignup() {
  ...
  try {
    ...
    showAlert('🎉 7-day premium trial started!', 'success', 3500);
  } catch (e) {
...
async function initiatePayment(planType) {
  ...
  if (!base) { showAlert('Backend URL missing. Set appConfig.apiBaseUrl in config.js', 'error'); return; }
  try {
...
function normalizeSpelling(s) {
  ...
  // common speech fillers/punctuation
  t = t.replace(/[^a-z\s]/g, ' ');

  // Collapse spaces/hyphens
  t = t.replace(/\s+/g, ' ').trim();

  // If user said the word (not letters), accept that too
  // But we primarily want "b e e" -> "bee"
  const tokens = t.split(' ');
  if (tokens.length > 1) {
    const lettersOnly = tokens.map(tok => tok[0]).join('');
    if (lettersOnly.length > 1) return lettersOnly;
  }
  return t.replace(/\s/g, ''); // fallback: remove spaces (e g -> eg)
}
