...
function checkOETAnswer(correctWord) {
  ...
  feedback.textContent = isCorrect ? "✓ Correct!" : `✗ Incorrect. Correct: ${correctWord}`;
  feedback.className = `feedback ${isCorrect ? 'correct' : 'incorrect'} visible`;

  // Auto-hide after 3 seconds (but stays visible while typing next word)
  setTimeout(() => {
    if(!document.activeElement.matches('#user-input')) {
...
document.getElementById('user-input').addEventListener('focus', () => {
  document.getElementById('feedback').classList.add('keyboard-active');
});

document.getElementById('user-input').addEventListener('blur', () => {
  document.getElementById('feedback').classList.remove('keyboard-active');
});